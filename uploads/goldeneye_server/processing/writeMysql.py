...
mydb = mysql.connector.connect(
  ...
  user="dbmasteruser",
  password="ConsultIT1!",
  database="goldenfacelog"
)


data_path = 'E:/My_Project/uploads/goldeneye_server/processing/unique_images'
images = []
images1 = []
...
def insert(photo, deviceid, target_file):
    ...
    try:
        if len(face) == 0:
            ...
            VALUES (%s, %s, %s, %s, %s)"
            val = (json_value1, Date_time, deviceid, faceIdNew, target_file)
            mycursor.execute(sql, val)
            mydb.commit()
            
            print(mycursor.rowcount, "record inserted.")
            text_file = open("E:/My_Project/uploads/goldeneye_server/Shahrukh/Logs/WriteMysql/{}.txt".format(str(today)), "a")
            ...
            text_file.close()
                ...
                for match in face:
                    ...
                    sql = "INSERT INTO GEYE_CAMERAFACESLOG (FACESTRINGAWS, DATETIMECREATED, CAMERAID, FACEIDAWS, PHOTOFILENAME) \
                    VALUES (%s, %s, %s, %s, %s)"
                    val = (json_value1, Date_time, deviceid, faceIdOld, target_file)
                    mycursor.execute(sql, val)
                    mydb.commit()
                    print(mycursor.rowcount, "record inserted.")
        ...
        VALUES (%s, %s, %s)"
        val = (Date_time, deviceid, target_file)
        mycursor.execute(sql, val)
        mydb.commit()
        end = time.time()
        time_elapsed = end - start
...
for i,file in enumerate(images):
    print(file)
    img = f"{Date_time}.jpg"
    device_id = 'IP43_8_990d8147'
    insert(file, device_id, img)
    os.replace(file, f'E:/My_Project/uploads/goldeneye_server/processing/done/{images1[i]}')
